...
const LikeItem = ({ like }) => {
	...
	const fetchLikeMutation = useMutation({
		mutationFn: (receiverId) =>
			makeAuthorizedRequest('/api/v1/member/like', 'post', {
		...
		<div className="bg-white px-[25px] pt-[16px] pb-[24px]">
			<div className="flex justify-between items-center mb-[16px]">
				<div className="flex items-center">
					<img
						className="w-[25px] h-[25px] mr-[4px] rounded-[50%]"
						src={like.photo_url}
					/>
				...
				{isLike ? (
					<button onClick={fetchDeleteLikeData}>
						<img src={RedHeart} className="w-[20px] h-[20px]" />
					</button>
				) : (
					<button onClick={fetchLikeData}>
						<img src={Heart} className="w-[20px] h-[20px]" />
					</button>
				)}
			</div>
