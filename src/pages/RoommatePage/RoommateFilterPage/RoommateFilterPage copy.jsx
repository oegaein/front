...
function RoommateFilterPageCopy() {
	...
	const handleChangeGenderValue = (value) => {
		...
		setFilters((prevFilters) => {
			...
			if (value === 'male') {
				updatedDormitory = {
					...prevFilters,
					dormitory: prevFilters.dormitory.filter(
						(item) => !['B', 'C', 'D'].includes(item),
					),
					roomType: [],
				};
	...
	const handleChange = (e) => {
		...
		if (
			(value === 'D' && filters.gender === 'female') ||
			(value === 'E' && filters.gender === 'male')
		) {
			!checked &&
				setFilters((prevFilters) => ({
					...prevFilters,
          ...
          if (Array.isArray(value)) {
			...
			<div className="filter-section">
				...
				<div className="flex">
					{[1, 2, 3, 4].map((number) => (
						<label
							className={`filter-input ${filters.peopleCount.includes(`${number}`) && 'selected'}`}
							key={number}
						>
							<input
								type="checkbox"
								name="peopleCount"
								value={number}
								checked={filters.peopleCount.includes(`${number}`)}
								onChange={handleChange}
								hidden
							/>
							{number}인
			...
			<div className="filter-section">
				...
				<label
					className={`filter-input ${filters.gender === 'male' && 'selected'}`}
				>
					<input
						type="radio"
						name="gender"
				...
				<label
					className={`filter-input ${filters.gender === 'female' && 'selected'}`}
				>
					<input
						type="radio"
						name="gender"
			...
			<div className="filter-section">
				...
				<div className="mb-[16px]">
					{['A', 'E'].map((dorm) => (
						<label
							key={dorm}
							className={`filter-input ${filters.dormitory.includes(dorm) && 'selected'} ${(!filters.gender || filters.gender === 'female') && 'disabled'}`}
						>
							<input
								type="checkbox"
								name="dormitory"
								value={dorm}
								checked={filters.dormitory.includes(dorm)}
								disabled={!filters.gender || filters.gender === 'female'}
								onChange={handleChange}
								hidden
							/>
							{dorm}동
							<div
								className={`disabled-line ${(!filters.gender || filters.gender === 'female') && 'disabled'}`}
							></div>
						</label>
					))}
				</div>
				...
				<div>
					{['B', 'C', 'D'].map((dorm) => (
						<label
							key={dorm}
							className={`filter-input ${filters.dormitory.includes(dorm) && 'selected'} ${(!filters.gender || filters.gender === 'male') && 'disabled'}`}
						>
							<input
								type="checkbox"
								name="dormitory"
								value={dorm}
								checked={filters.dormitory.includes(dorm)}
								disabled={!filters.gender || filters.gender === 'male'}
								onChange={handleChange}
								hidden
							/>
							{dorm}동
							<div
								className={`disabled-line ${(!filters.gender || filters.gender === 'male') && 'disabled'}`}
							></div>
						</label>
					))}
				</div>
			...
			{filters.dormitory.some((i) => ['D', 'E'].includes(i)) && (
				<div className="filter-section">
					...
					{['2인실', '4인실'].map((roomType) => (
						<label
							className={`filter-input ${filters.roomType.includes(roomType) && 'selected'}`}
						>
							<input
								type="checkbox"
								name="roomType"
								value={roomType}
								checked={filters.roomType.includes(roomType)}
								onChange={handleChange}
								hidden
							/>
							{roomType}
			...
			<div className="filter-section">
				...
				{showAllMbtis ? (
					...
					<button className="purple-btn mt-[8px]" onClick={handleShowAllMbtis}>
						필터 더보기
					</button>
				)}
			</div>
				...
				<div className="flex flex-wrap content-between h-[84px]">
					{['코골이형', '이갈이형', '잠꼬대형', '무소음형'].map(
						(sleepHabit) => (
							<label
								key={sleepHabit}
								className={`filter-input ${filters.sleepHabit.includes(sleepHabit) && 'selected'}`}
							>
								<input
									type="checkbox"
									name="sleepHabit"
									value={sleepHabit}
									checked={filters.sleepHabit.includes(sleepHabit)}
									onChange={handleChange}
									hidden
								/>
								{sleepHabit}
			...
			<div className="filter-section">
				...
				<div className="flex">
					{['흡연', '비흡연'].map((isSmoker) => (
						<label
							className={`filter-input ${filters.isSmoker.includes(isSmoker) && 'selected'}`}
						>
							<input
								type="checkbox"
								name="isSmoker"
								value={isSmoker}
								checked={filters.isSmoker.includes(isSmoker)}
								onChange={handleChange}
								hidden
							/>
							{isSmoker}
			...
			<div className="filter-section">
				...
				<div className="flex">
					{['집순이', '밖순이'].map((goOut) => (
						<label
							className={`filter-input ${filters.goOut.includes(goOut) && 'selected'}`}
						>
							<input
								type="checkbox"
								name="goOut"
								value={goOut}
								checked={filters.goOut.includes(goOut)}
								onChange={handleChange}
								hidden
							/>
							{goOut}
			...
			<div className="filter-section">
				...
				<div className="flex">
					{['예민한편', '둔감한편'].map((soundSensitivity) => (
						<label
							className={`filter-input ${filters.soundSensitivity.includes(soundSensitivity) && 'selected'}`}
						>
							<input
								type="checkbox"
								name="soundSensitivity"
								value={soundSensitivity}
								checked={filters.soundSensitivity.includes(soundSensitivity)}
								onChange={handleChange}
								hidden
							/>
							{soundSensitivity}
			...
			<div className="flex items-center justify-between h-[91px] px-[25px]">
				<button onClick={handleClickResetBtn} className="reset-btn">
					초기화
				</button>
				<button onClick={handleClickFilterBtn} className="filter-btn">
					350개 글보기
				</button>
			</div>
		</SettingStyle>
