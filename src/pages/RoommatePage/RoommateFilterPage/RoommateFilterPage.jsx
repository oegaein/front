...
function RoommateFilterPage() {
	...
	const handleScroll = throttle(() => {
		...
		setIsLowerBarVisible(
			prevScrollPos > currentScrollPos || currentScrollPos < 10 || isBottom,
		);
			...
			if (checked) {
				if (name === 'sort') {
					updatedSelectedFilters = [
						{ name, value },
						...prevFilters.filter((item) => item.name !== 'sort'),
					];
				} else {
				...
				if (['minAge', 'maxAge'].includes(name)) {
					updatedSelectedFilters = prevFilters.filter(
						(item) => item.name !== 'minAge' && item.name !== 'maxAge',
					);
				} else if (['minYear', 'maxYear'].includes(name)) {
					updatedSelectedFilters = prevFilters.filter(
						(item) => item.name !== 'minYear' && item.name !== 'maxYear',
					);
				} else {
					updatedSelectedFilters = prevFilters.filter(
	...
	const handleChangeGenderValue = (e) => {
		...
		setFilters((prevFilters) => {
			...
			if (value === '남성') {
				updateddong = {
					...prevFilters,
					dong: prevFilters.dong.filter(
						(item) => !['B동', 'C동', 'D동'].includes(item),
					),
					roomSize: [],
					gender: value,
				};
	...
	const handleChangeDong = (e) => {
		...
		if (
			(value === 'D동' && filters.gender === '여성') ||
			(value === 'E동' && filters.gender === '남성')
		) {
			!checked &&
				setFilters((prevFilters) => ({
					...prevFilters,
				...
				<section>
					{selectedFilters.length > 0 && (
						<div className="tag-container bg-white flex gap-[10px] sticky z-10 top-0 overflow-x-scroll pt-[10px] pb-[25px] pl-[25px]">
							{selectedFilters.map((item, index) => {
								// Age 또는 Year 필터인지 확인
								const isAgeFilter =
									item.name === 'minAge' || item.name === 'maxAge';
								const isYearFilter =
									item.name === 'minYear' || item.name === 'maxYear';

								// minAge, maxAge, minYear, maxYear 값을 찾음
								const minAge =
									selectedFilters.find((filter) => filter.name === 'minAge')
										?.value ?? getInitialValueByName('minAge');
								const maxAge =
									selectedFilters.find((filter) => filter.name === 'maxAge')
										?.value ?? getInitialValueByName('maxAge');
								const minYear =
									selectedFilters.find((filter) => filter.name === 'minYear')
										?.value ?? getInitialValueByName('minYear');
								const maxYear =
									selectedFilters.find((filter) => filter.name === 'maxYear')
										?.value ?? getInitialValueByName('maxYear');
									...
									<>
										{item.name === 'maxAge' && (
											...
											>
												{minAge}세 ~ {maxAge}세
												<img
													className="ml-[5px] h-[12px] w-[12px]"
													src={Close}
													alt="close button"
												/>
											</button>
					...
					<div className="filter-section">
						...
						<div className="flex">
							{['1인', '2인', '3인', '4인'].map((number, index) => (
								<label
									className={`filter-input ${filters.targetNumberOfPeople.includes(`${number}`) && 'selected'}`}
									key={index}
								>
									<input
										type="checkbox"
										name="targetNumberOfPeople"
										data-testid={number}
										value={number}
										checked={filters.targetNumberOfPeople.includes(`${number}`)}
										onChange={handleChange}
										hidden
									/>
									{number}
					...
					<div className="filter-section">
						...
						{['남성', '여성'].map((gender, index) => (
							<label
								className={`filter-input ${filters.gender === gender && 'selected'}`}
								key={index}
							>
								<input
									type="radio"
					...
					<div className="filter-section">
						...
						<div className="mb-[16px]">
							{['A동', 'E동'].map((dorm, index) => (
								<label
									key={index}
									className={`filter-input ${filters.dong.includes(dorm) && 'selected'} ${(!filters.gender || filters.gender === '여성') && 'disabled'}`}
								>
									<input
										type="checkbox"
										name="dong"
										value={dorm}
										checked={filters.dong.includes(dorm)}
										disabled={!filters.gender || filters.gender === '여성'}
										onChange={handleChangeDong}
										hidden
									/>
									{dorm}
									<div
										className={`disabled-line ${(!filters.gender || filters.gender === '여성') && 'disabled'}`}
									></div>
								</label>
							))}
						</div>
						...
						<div>
							{['B동', 'C동', 'D동'].map((dorm, index) => (
								<label
									key={index}
									className={`filter-input ${filters.dong.includes(dorm) && 'selected'} ${(!filters.gender || filters.gender === '남성') && 'disabled'}`}
								>
									<input
										type="checkbox"
										name="dong"
										value={dorm}
										checked={filters.dong.includes(dorm)}
										disabled={!filters.gender || filters.gender === '남성'}
										onChange={handleChangeDong}
										hidden
									/>
									{dorm}
									<div
										className={`disabled-line ${(!filters.gender || filters.gender === '남성') && 'disabled'}`}
									></div>
								</label>
							))}
						</div>
					...
					{filters.dong.some((i) => ['D동', 'E동'].includes(i)) && (
						<div className="filter-section">
							...
							{['2인실', '4인실'].map((roomSize, index) => (
								<label
									key={index}
									className={`filter-input ${filters.roomSize.includes(roomSize) && 'selected'}`}
								>
									<input
										type="checkbox"
										name="roomSize"
										value={roomSize}
										checked={filters.roomSize.includes(roomSize)}
										onChange={handleChange}
										hidden
									/>
									{roomSize}
					...
					<div className="filter-section">
						...
						<div className="flex flex-wrap content-between h-[84px] mb-[24px]">
							{iMbtis.map((mbti, index) => (
								<label
									className={`filter-input w-[73px] ${filters.mbti.includes(mbti) && 'selected'}`}
									key={index}
								>
									<input
										type="checkbox"
						...
						{showAllMbtis ? (
							<>
								...
								<button className="purple-btn" onClick={handleShowAllMbtis}>
									닫기
								</button>
							</>
						) : (
							<button
								className="purple-btn mt-[8px]"
								onClick={handleShowAllMbtis}
							>
								필터 더보기
							</button>
						)}
					...
					<div className="filter-section">
						...
						<div className="flex flex-wrap content-between h-[84px]">
							{['코골이형', '이갈이형', '잠꼬대형', '무소음형'].map(
								(sleepingHabits, index) => (
									<label
										key={index}
										className={`filter-input ${filters.sleepingHabits.includes(sleepingHabits) && 'selected'}`}
									>
										<input
											type="checkbox"
											name="sleepingHabits"
											value={sleepingHabits}
											checked={filters.sleepingHabits.includes(sleepingHabits)}
											onChange={handleChange}
											hidden
										/>
										{sleepingHabits}
					...
					<div className="filter-section">
						...
						<div className="flex">
							{['아침형', '저녁형'].map((lifePattern, index) => (
								<label
									className={`filter-input ${filters.lifePattern.includes(lifePattern) && 'selected'}`}
									key={index}
								>
									<input
										type="checkbox"
										name="lifePattern"
										value={lifePattern}
										checked={filters.lifePattern.includes(lifePattern)}
										onChange={handleChange}
										hidden
									/>
									{lifePattern}
					...
					<div className="filter-section">
						...
						<div className="flex">
							{['흡연', '비흡연'].map((smoking, index) => (
								<label
									className={`filter-input ${filters.smoking.includes(smoking) && 'selected'}`}
									key={index}
								>
									<input
										type="checkbox"
					...
					<div className="filter-section">
						...
						<div className="flex flex-wrap content-between h-[84px]">
							{['매일', '주 1회 이상', '월 1회 이상', '생각날 때 가끔'].map(
								(cleaningCycle, index) => (
									<label
										className={`filter-input ${filters.cleaningCycle.includes(cleaningCycle) && 'selected'}`}
										key={index}
									>
										<input
											type="checkbox"
											name="cleaningCycle"
											value={cleaningCycle}
											checked={filters.cleaningCycle.includes(cleaningCycle)}
											onChange={handleChange}
											hidden
										/>
										{cleaningCycle}
					...
					<div className="filter-section">
						...
						<div className="flex">
							{['집순이', '밖순이'].map((outing, index) => (
								<label
									className={`filter-input ${filters.outing.includes(outing) && 'selected'}`}
									key={index}
								>
									<input
										type="checkbox"
										name="outing"
										value={outing}
										checked={filters.outing.includes(outing)}
										onChange={handleChange}
										hidden
									/>
									{outing}
					...
					<div className="filter-section">
						...
						<div className="flex">
							{['예민한편', '둔감한편'].map((sensitivity, index) => (
								<label
									className={`filter-input ${filters.sensitivity.includes(sensitivity) && 'selected'}`}
									key={index}
								>
									<input
										type="checkbox"
										name="sensitivity"
										value={sensitivity}
										checked={filters.sensitivity.includes(sensitivity)}
										onChange={handleChange}
										hidden
									/>
									{sensitivity}
					...
					<div
						className={`filter-section bg-white z-50 fixed bottom-0 flex items-center justify-between gap-[10px] h-[91px]
        transition-transform duration-300 ${isLowerBarVisible ? 'translate-y-0' : 'translate-y-full'}`}
					>
						<button
							onClick={handleClickResetBtn}
							className="reset-btn whitespace-nowrap"
						>
							초기화
						</button>
						<button
